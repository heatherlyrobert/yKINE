...
char  /*--> save relative ik based move -----------[ ------ [ ------ ]-*/
ykine__scrp_fk_getter       (int a_leg, int a_seg, float *a_deg)
{
   /*---(locals)-----------+-----+-----+-*/
   char        rce         =  -10;               /* return code for errors    */
   char        rc          =    0;
...
ykine_scrp_fk           (void)
{
   ...
   for (x_leg = 0; x_leg < YKINE_MAX_LEGS; ++x_leg) {
      ...
      /*---(adjust angles)---------------*/
      rc = ykine__scrp_fk_getter (x_leg, YKINE_FEMU, &f);
      rc = ykine__scrp_fk_getter (x_leg, YKINE_PATE, &p);
      rc = ykine__scrp_fk_getter (x_leg, YKINE_TIBI, &t);
      /*---(calculate endpoint)----------*/
      rc = yKINE_forward     (x_leg, f, p, t);
      DEBUG_YKINE_SCRP  yLOG_value   ("forward"   , rc);
      /*---(process moves)---------------*/
      rc = ykine__scrp_set_servo (YKINE_FK, x_leg, YKINE_FEMU, f, b);
      rc = ykine__scrp_set_servo (YKINE_FK, x_leg, YKINE_PATE, p, b);
      rc = ykine__scrp_set_servo (YKINE_FK, x_leg, YKINE_TIBI, t, b);
      /*---(done)------------------------*/
   }
   /*---(complete)-----------------------*/
   DEBUG_YKINE_SCRP   yLOG_exit    (__FUNCTION__);
...
char  /*--> save relative ik based move -----------[ ------ [ ------ ]-*/
ykine__scrp_ik_getter   (int a_leg, float *x, float *z, float *y)
{
   /*---(locals)-----------+-----+-----+-*/
   char        rce         =  -10;               /* return code for errors    */
   char        rc          =    0;
...
ykine_scrp_ik           (void)
{
   ...
   for (x_leg = 0; x_leg < YKINE_MAX_LEGS; ++x_leg) {
      ...
      /*---(adjust positions)------------*/
      rc = ykine__scrp_ik_getter (x_leg, &x, &z, &y);
      if (rc <  0) {
